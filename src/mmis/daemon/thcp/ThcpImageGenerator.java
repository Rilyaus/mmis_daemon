...
import mmis.daemon.util.legend.ThcpLegend;
...
public class ThcpImageGenerator extends ThcpFileGenerator {
	
	private final int imageExpandFactor = 30;
	private final int imageResizeFactor = 1;
	
	public ThcpImageGenerator(final String metaDirPath) {	
		
		this.setThcpMetaFiles(metaDirPath, ThcpFileGenerator.THCP_MODEL.THCP);
	...
	public boolean initCoordinates() {
		...
		this.modelGridUtil = new ModelGridUtil(ModelGridUtil.Model.THCP, ModelGridUtil.Position.MIDDLE_CENTER, this.thcpLatFilePath, this.thcpLonFilePath);
	...
	public List<Map<String, Object>> generateFile(final String srcFilePath, final String destFilePath) {
		...
		final Object[][] variableInfos = new Object[][]{
			{"heat_content", "HEAT", ThcpLegend.Legend.HEAT, 75, null, 0}
		};
		
		SimpleDateFormat issuedTmFormat = new SimpleDateFormat("yyyyMMdd");
		SimpleDateFormat fcstTmFormat = new SimpleDateFormat("yyyyMMddHH");
		
		String issuedTmStr = new File(srcFilePath).getName().split("\\.")[0];
		...
		try {
			...
			for(Object[] variableInfo : variableInfos) {
				...
				ThcpLegend thcpLegend = ThcpLegend.getLegend((ThcpLegend.Legend)variableInfo[2]);
				...
				for(int t=0 ; t<timeLength ; t++) {
				    ...
				    String imgFileName = "THCP_" + variableInfo[1] + "_" + issuedTmStr + "_"+ String.format("%02d", t+1) + ".png";
