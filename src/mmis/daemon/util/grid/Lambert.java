...
{
    public enum GRID_PARAM_TYPE { GK2A, DFS, DFS2 }
    ...
    private lamc_parameter _getPredefinedGridValue(GRID_PARAM_TYPE gridParamType)
    {
        ...
        if (gridParamType == GRID_PARAM_TYPE.GK2A) 
        {
            m_map.Re = 6371.00877; // Earth radius (km) [사용할 지구 반경]
            m_map.grid = 2.0; // for 6 km [격자간격] // LC 1.5 // 6
            ...
            m_map.xo = 3000/m_map.grid; 
            m_map.yo = 2600/m_map.grid; 
        }
        else if (gridParamType == GRID_PARAM_TYPE.DFS) {
        	
        	m_map.Re = 6371.00877; // Earth radius (km) [사용할 지구 반경]
            m_map.grid = 1.0; // for 6 km [격자간격] // LC 1.5 // 6
            m_map.slat1 = 30.0; // Standard latitude 1 [표준 위도1] (최하단 위도)
            m_map.slat2 = 60.0; // Standard latitude 2 [표준 위도2] // TRUE_LAT_NORTH (최상단 위도)
            m_map.center_lat = 38.0; // latitude of known point in map (degree) [기준점의 위도]
            m_map.center_lon = 126.0; // longitude of known point in map (degree) [기준점의 경도]
            m_map.xo = 366/m_map.grid; 
            m_map.yo = 771/m_map.grid; 
        }
        else if (gridParamType == GRID_PARAM_TYPE.DFS2) {
        	
        	m_map.Re = 6371.00877; // Earth radius (km) [사용할 지구 반경]
            m_map.grid = 5.0; // for 6 km [격자간격] // LC 1.5 // 6
            m_map.slat1 = 30.0; // Standard latitude 1 [표준 위도1] (최하단 위도)
            m_map.slat2 = 60.0; // Standard latitude 2 [표준 위도2] // TRUE_LAT_NORTH (최상단 위도)
            m_map.center_lat = 38.0; // latitude of known point in map (degree) [기준점의 위도]
            m_map.center_lon = 126.0; // longitude of known point in map (degree) [기준점의 경도]
            m_map.xo = 210/m_map.grid-0.5; 
            m_map.yo = 675/m_map.grid-1.5; 
        }
    ...
    private void _init(lamc_parameter baseParam)
    {
        ...
        _getPredefinedGridValue(GRID_PARAM_TYPE.GK2A);
    }
