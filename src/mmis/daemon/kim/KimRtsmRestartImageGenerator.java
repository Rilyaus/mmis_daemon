...
public class KimRtsmRestartImageGenerator extends KimFileGenerator {
	
	private final int imageExpandFactor = 100;
	private final int imageResizeFactor = 1;
	
	public KimRtsmRestartImageGenerator(final String metaDirPath) {	
		
		this.setKimMetaFiles(metaDirPath, KimFileGenerator.KIM_MODEL.KIM_RTSM_RESTART);
	...
	public boolean initCoordinates() {
		...
		this.modelGridUtil = new ModelGridUtil(ModelGridUtil.Model.KIM_RTSM_RESTART, ModelGridUtil.Position.MIDDLE_CENTER, this.kimLatFilePath, this.kimLonFilePath);
	...
	public List<Map<String, Object>> generateFile(final String srcFilePath1, final String srcFilePath2, final String destFilePath) {
		...
		final Object[][] variableInfos = new Object[][]{
			{"sshn", "SURG_HEIGHT", KimLegend.Legend.RTSM_SURG_HEIGHT, 1, null, 0, 0}
		};
		...
		String issuedTmStr = new File(srcFilePath1).getName().split("\\.")[0].split("_")[2];
		int fcstHour = Integer.valueOf(new File(srcFilePath1).getName().split("\\.")[0].split("_")[3]);
		...
		try {
			...
			for(Object[] variableInfo : variableInfos) {
				...
				for(int t=0 ; t<timeLength ; t++) {
				    ...
				    String imgFileName = "KIM_RTSM_RESTART_" + issuedTmStr + "_" + variableInfo[1] + "_" + String.format("%02d", fcstHour+1) + ".png";
