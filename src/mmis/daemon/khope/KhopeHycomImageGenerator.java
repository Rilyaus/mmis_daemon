...
public class KhopeHycomImageGenerator extends KhopeFileGenerator {
	
	private final int imageExpandFactor = 50;
	private final int imageResizeFactor = 1;
	
	public KhopeHycomImageGenerator(final String metaDirPath) {	
		
		this.setKhopeMetaFiles(metaDirPath, KhopeFileGenerator.KHOPE_MODEL.KHOPE_HYCOM);
	...
	public boolean initCoordinates() {
		...
		this.modelGridUtil = new ModelGridUtil(ModelGridUtil.Model.KHOPE_HYCOM, ModelGridUtil.Position.MIDDLE_CENTER, this.khopeLatFilePath, this.khopeLonFilePath);
	...
	public List<Map<String, Object>> generateFile(final String srcFilePath, final String destFilePath) {
		...
		final Object[][] variableInfos = new Object[][]{
			{"salinity", "SALT", KhopeLegend.Legend.HYCOM_SALT, 1, 40},
			{"water_temp", "TEMP", KhopeLegend.Legend.HYDR_TEMP, 1, 40}
		};
			...
			for(Object[] variableInfo : variableInfos) {
				...
				for(int t=0 ; t<timeLength ; t++) {
					...
					for(int d=0 ; d<depLength ; d++) {
					    ...
					    String imgFileName = "KHOPE_HYCOM_" + issuedTmStr + "_" +  variableInfo[1] + "_" + String.format("%02d", t+1) + "_" + String.format("%02d", d+1) + ".png";
