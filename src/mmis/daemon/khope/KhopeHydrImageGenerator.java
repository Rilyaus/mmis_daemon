...
public class KhopeHydrImageGenerator extends KhopeFileGenerator {
	
	private final int imageExpandFactor = 300;
	private final int imageResizeFactor = 1;
	
	public KhopeHydrImageGenerator(final String metaDirPath) {	
		
		this.setKhopeMetaFiles(metaDirPath, KhopeFileGenerator.KHOPE_MODEL.KHOPE_HYDR);
	...
	public boolean initCoordinates() {
		...
		this.modelGridUtil = new ModelGridUtil(ModelGridUtil.Model.KHOPE_HYDR, ModelGridUtil.Position.MIDDLE_CENTER, this.khopeLatFilePath, this.khopeLonFilePath);
	...
	public List<Map<String, Object>> generateFile(final String srcFilePath, final String destFilePath) {
		...
		final Object[][] variableInfos = new Object[][]{
			{"Results/salinity/salinity", "SALT", KhopeLegend.Legend.HYDR_SALT, 30, 19},
			{"Results/temperature/temperature", "TEMP", KhopeLegend.Legend.HYDR_TEMP, 30, 19}
		};
		...
		try {
			...
			for(Object[] variableInfo : variableInfos) {
				...
				for(int t=0 ; t<timeLength ; t++) {
					...
					Variable var = ncFile.findVariable((String)variableInfo[0] + "_" + String.format("%05d", t+1));
					
					for(int d=0 ; d<depLength ; d++) {
						
						if(this.testMode && d < depLength-1) {
							continue;
						}
						...
						double[][] values = GridCalcUtil.convertReverseStorageToValues(var.read(rangeList).getStorage(), rows, cols, false);
					    ...
					    String imgFileName = "KHOPE_HYDR_" + issuedTmStr + "_" +  variableInfo[1] + 
					    		"_" + String.format("%02d", t+1) + "_" + String.format("%02d", d+1) + ".png";
