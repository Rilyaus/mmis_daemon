...
public class KhopeGlosea5DayNewImageGenerator extends KhopeFileGenerator {
	...
	public KhopeGlosea5DayNewImageGenerator(final String metaDirPath) {	
		
		this.setKhopeMetaFiles(metaDirPath, KhopeFileGenerator.KHOPE_MODEL.KHOPE_GLOSEA5_DAY_NEW);
	...
	public boolean initCoordinates() {
		...
		this.modelGridUtil = new ModelGridUtil(ModelGridUtil.Model.KHOPE_GLOSEA5_DAY_NEW, ModelGridUtil.Position.TOP_LEFT, this.khopeLatFilePath, this.khopeLonFilePath);
		
		this.latBuffer = this.modelGridUtil.getLatBuffer();
		this.lonBuffer = this.modelGridUtil.getLonBuffer();
		
		this.modelRegridUtil = new ModelGridUtil(ModelGridUtil.Model.KHOPE_GLOSEA5_DAY_NEW_REGRID, ModelGridUtil.Position.TOP_LEFT, this.khopeRegridLatFilePath, this.khopeRegridLonFilePath);
	...
	public List<Map<String, Object>> generateFile(final String srcFilePath, final String destFilePath) {
		...
		// GLOSEA5_DAY_NEW 모델은 누락되는 격자가 있으므로 값도 아래와 같은 제한으로 읽어야한다
			...
			for(Object[] variableInfo : variableInfos) {
				...
				for(int t=0 ; t<timeLength ; t++) {
					...
					for(int d=0 ; d<(depLength<0?1:depLength) ; d++) {
					    ...
					    String imgFileName = "KHOPE_GLOSEA5_DAY_NEW_" + issuedTmStr + "_" +  variableInfo[1] + "_" + 
